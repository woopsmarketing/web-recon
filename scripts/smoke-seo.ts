 ...
 *   7b entity-bearing upstream title: every encoding rewritten, none survives
...
/** A fixture upstream that answers every path with one HTML document. */
async function startHtmlUpstream(html: string): Promise<{ server: Server; baseUrl: string }> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // ---- 7b entity-bearing upstream title (GED-E) ------------------------
    // Section 7's fixture title has no escapable character, so it has exactly
    // ONE encoding — which is why a literal split/join looked correct for five
    // tasks. A real source title does not: see
    // data/domainchecker.co.kr/production-seo-plans/2026-08-19T07-23-36-101Z/
    // report/qa.json, where "블로그 · 도메인체커 | 도메인 투자 & SEO 가이드 |
    // 도메인체커" survived into the browser because the `&` is `&amp;` in the
    // SSR'd markup and `\u0026` in the flight payload.
    //
    // Fixture-strength rule for the absence assertions below: every encoding
    // must sit in at least one position the PRE-FIX code would have LEFT
    // BEHIND, or its absence assertion proves nothing. The pre-fix code was
    // (a) `<title>[^<]*</title>` → the plan title, then (b) a split/join on
    // the RAW upstream title. An encoding placed only inside <title> is cleaned
    // by (a) and does not discriminate — which is why the React hex-apostrophe
    // form also sits in SSR'd body text below. `raw` is the one encoding that
    // cannot be made discriminating: it IS the needle of (b), so no position
    // survives it; its assertion is a forward regression guard, not evidence
    // for this fix.
    section("7b entity-bearing upstream title survives in no encoding (GED-E)");
    const entitySourceTitle = `Ace & Co <Guide> "Best" Bob's Tips`;
    const entityPlanTitle = "새브랜드 & Co.";
    // Hand-written (NOT generated by the code under test): the encodings a
    // Next.js response gives that title.
    const sourceVariants: Record<string, string> = {
      raw: `Ace & Co <Guide> "Best" Bob's Tips`,
      htmlHexApostrophe: `Ace &amp; Co &lt;Guide&gt; &quot;Best&quot; Bob&#x27;s Tips`,
      htmlDecimalApostrophe: `Ace &amp; Co &lt;Guide&gt; &quot;Best&quot; Bob&#39;s Tips`,
      htmlNamedApostrophe: `Ace &amp; Co &lt;Guide&gt; &quot;Best&quot; Bob&apos;s Tips`,
      json: `Ace & Co <Guide> \\"Best\\" Bob's Tips`,
      jsonHtmlEscaped: `Ace \\u0026 Co \\u003cGuide\\u003e \\"Best\\" Bob's Tips`,
    };
    check(
      "7b escapeVariants: 6 distinct encodings for an entity title, 1 for an ASCII title",
      escapeVariants(entitySourceTitle).length === ESCAPE_VARIANT_ENCODINGS.length &&
        escapeVariants("Root Original Title").length === 1,
      JSON.stringify(escapeVariants(entitySourceTitle).map((v) => v.encoding)),
    );
    check(
      "7b escapeVariants encodings match the hand-written fixture forms",
      escapeVariants(entitySourceTitle)
        .map((v) => v.text)
        .sort()
        .join("|") === Object.values(sourceVariants).sort().join("|"),
      JSON.stringify(escapeVariants(entitySourceTitle)),
    );
    const entityUpstreamHtml =
      `<!DOCTYPE html><html><head><title>${sourceVariants.htmlHexApostrophe}</title>` +
      `<meta property="og:title" content="${sourceVariants.htmlDecimalApostrophe}"/>` +
      `<meta name="twitter:title" content="${sourceVariants.htmlNamedApostrophe}"/>` +
      // React's escapeTextForBrowser hex-apostrophe form in SSR'd body text:
      // the position that makes the htmlHexApostrophe absence assertion
      // discriminating (inside <title> alone, the pre-fix regex cleaned it).
      `</head><body><h1>${sourceVariants.htmlHexApostrophe}</h1>` +
      `<p>${sourceVariants.htmlNamedApostrophe}</p>` +
      `<script>self.__next_f=[];self.__next_f.push([1,"t:\\"${sourceVariants.jsonHtmlEscaped}\\""]);</script>` +
      `<script type="application/json">{"title":"${sourceVariants.json}"}</script>` +
      "<script>var wrRaw = `" +
      sourceVariants.raw +
      "`;</script>" +
      `<script>document.title = "${sourceVariants.json}";</script></body></html>`;
    // Fixture-strength replay hook: dump the exact fixture bytes so a
    // reconstruction of the pre-fix rewriteHtmlHead can be replayed over them
    // outside this repo, proving each absence assertion discriminates. Off
    // unless WR_SEO_FIXTURE_OUT is set; never written by a normal run.
    if (process.env.WR_SEO_FIXTURE_OUT !== undefined) {
      await writeFile(process.env.WR_SEO_FIXTURE_OUT, entityUpstreamHtml, "utf8");
    }
    const entityEntry = { upstreamTitle: entitySourceTitle, title: entityPlanTitle, headHtml: "" };
    const entityRewritten = rewriteHtmlHead(entityUpstreamHtml, entityEntry);
    for (const [encoding, variant] of Object.entries(sourceVariants)) {
      check(
        `7b source title in ${encoding} encoding does not survive rewriteHtmlHead`,
        !entityRewritten.includes(variant),
        variant,
      );
    }
    check(
      "7b <title> element carries the plan title, HTML-escaped",
      entityRewritten.includes("<title>새브랜드 &amp; Co.</title>"),
      entityRewritten.slice(0, 120),
    );
    check(
      "7b HTML-escaped occurrence gets an HTML-escaped replacement",
      entityRewritten.includes(`<meta property="og:title" content="새브랜드 &amp; Co."/>`) &&
        entityRewritten.includes("<p>새브랜드 &amp; Co.</p>"),
    );
    check(
      "7b SSR'd body text in the React hex-apostrophe encoding is rewritten",
      entityRewritten.includes("<h1>새브랜드 &amp; Co.</h1>"),
      entityRewritten,
    );
    check(
      "7b flight-payload occurrence gets a JSON+html-escaped replacement",
      entityRewritten.includes(`push([1,"t:\\"새브랜드 \\u0026 Co.\\""])`),
    );
    check(
      "7b raw occurrence gets a raw replacement",
      entityRewritten.includes("var wrRaw = `새브랜드 & Co.`;"),
    );
    check(
      "7b no double-escaping artifact anywhere in the rewritten body",
      !entityRewritten.includes("&amp;amp;") &&
        !entityRewritten.includes("&amp;lt;") &&
        !entityRewritten.includes("&amp;quot;") &&
        !entityRewritten.includes("\\\\u0026") &&
        !entityRewritten.includes("\\u0026amp;"),
    );
    const entityUpstream = await startHtmlUpstream(entityUpstreamHtml);
    const homeRendered = renderedHead.routes.find((r) => r.route === "/") as RenderedHead["routes"][number];
    const entityRenderedHead: RenderedHead = {
      schemaVersion: 1,
      routes: [{ ...homeRendered, upstreamTitle: entitySourceTitle, title: entityPlanTitle }],
    };
    const entityProxy = await startSeoProxy(entityUpstream.baseUrl, {
      renderedHead: entityRenderedHead,
      robotsTxt: robotsPreview,
      sitemapXml: null,
    });
    try {
      const entityServed = await (await fetch(`${entityProxy.baseUrl}/`)).text();
      const survivors = Object.entries(sourceVariants)
        .filter(([, variant]) => entityServed.includes(variant))
        .map(([encoding]) => encoding);
      check("7b served preview body carries no encoding of the source title", survivors.length === 0, survivors.join(","));
      check(
        "7b served preview <title> is the plan title, HTML-escaped",
        entityServed.includes("<title>새브랜드 &amp; Co.</title>"),
      );
      check(
        "7b served preview has no double-escaping artifact",
        !entityServed.includes("&amp;amp;") && !entityServed.includes("\\\\u0026"),
      );
      const entityBrowser = await chromium.launch();
      try {
        const page = await entityBrowser.newPage();
        await page.goto(`${entityProxy.baseUrl}/`, { waitUntil: "load", timeout: 30_000 });
        await page.waitForTimeout(300);
        const entityTitle = (await page.evaluate("document.title")) as string;
        check("7b Chromium document.title is the plan title, entity decoded once", entityTitle === entityPlanTitle, entityTitle);
      } finally {
        await entityBrowser.close();
      }
    } finally {
      await entityProxy.stop();
      await new Promise<void>((resolve) => entityUpstream.server.close(() => resolve()));
    }
