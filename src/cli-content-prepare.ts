...
import { prepareContentRun } from "./content-injection/index.js";

/**
 * web-recon Content Injection — prepare CLI (Task 19 §2/§4).
 *
...
async function main(): Promise<void> {
  ...
  if (!args.templateManifestFile || (!args.intent && !args.intentFile)) {
    printUsage();
    process.exitCode = 1;
    return;
  }
  const rawIntent = args.intent ?? (await readFile(args.intentFile!, "utf8")).trim();
  ...
  console.log(`[content:prepare] batches   ${prepared.request.batches.length}`);
  console.log("");
